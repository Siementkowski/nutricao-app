...
function CustomTooltip({ active, payload, label }: TooltipProps) {
  ...
  return (
    <div
      ...
      style={{ backgroundColor: '#FBFAF4', border: '1px solid #E2DBC9', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}
    >
      <p className="text-xs mb-0.5" style={{ color: '#8B8170', fontWeight: 300 }}>{label}</p>
      <p className="text-sm" style={{ color: '#20302A', fontWeight: 600 }}>{payload[0].value} kg</p>
    </div>
  )
}
    ...
    <>
      ...
      <div>
        ...
        <div className="px-5 space-y-4 pb-28">
          ...
          >
            <p className="text-xs uppercase tracking-wider mb-4" style={{ color: '#8B8170', fontWeight: 300 }}>
            ...
            {weightLogs.length < 2 ? (
              ...
              >
                <p className="text-xs text-center" style={{ color: '#8B8170', fontWeight: 300 }}>
                  {weightLogs.length === 0
              ...
              <ResponsiveContainer width="100%" height={180}>
                <LineChart data={chartData} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#E2DBC9" vertical={false} />
                  <XAxis
                    dataKey="date"
                    tick={{ fontSize: 10, fill: '#8B8170' }}
                    tickLine={false}
                    axisLine={false}
                  ...
                  <Line
                    ...
                    strokeWidth={2}
                    dot={false}
                    activeDot={{ r: 4, fill: '#3B6B4D', strokeWidth: 0 }}
                  />
                </LineChart>
              </ResponsiveContainer>
            )}
              ...
              <div className="grid grid-cols-4 gap-2 mt-4">
                {[
                  ...
                  <div
                    ...
                    style={{ backgroundColor: '#EFEADD' }}
                  >
                    <p className="text-xs mb-1" style={{ color: '#8B8170', fontWeight: 300 }}>{label}</p>
                    <p className="text-sm" style={{ color: '#20302A', fontWeight: 600 }}>{value}</p>
                  </div>
                ))}
              </div>
            )}
          ...
          >
            ...
            <div className="flex flex-wrap gap-3 mb-4">
              {Object.entries(STATUS_LABEL).map(([key, label]) => (
                <div key={key} className="flex items-center gap-1.5">
                  ...
                  />
                  <span className="text-xs" style={{ color: '#8B8170', fontWeight: 300 }}>{label}</span>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                >
                  <p className="text-xs mb-1" style={{ color: '#8B8170', fontWeight: 300 }}>{label}</p>
                  <p className="text-lg" style={{ color: '#46774F', fontWeight: 600 }}>
                    {value}
                    <span className="text-xs ml-0.5" style={{ fontWeight: 300, color: '#8B8170' }}>{unit}</span>
                  </p>
                </div>
              ))}
            </div>
