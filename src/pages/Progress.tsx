...
function CustomTooltip({ active, payload, label }: TooltipProps) {
  ...
  return (
    <div
      ...
      style={{ backgroundColor: '#152318', border: '1px solid #22362A', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}
    >
      <p className="text-xs mb-0.5" style={{ color: '#8AAF8C', fontWeight: 300 }}>{label}</p>
      <p className="text-sm" style={{ color: '#EAF2E6', fontWeight: 600 }}>{payload[0].value} kg</p>
    </div>
  )
}
    ...
    <>
      ...
      <div>
        <PageHeader
          ...
          action={
            <button
              ...
              style={{ backgroundColor: '#1C3520', color: '#72C45A', fontWeight: 500 }}
            >
        ...
        <div className="px-5 space-y-4 pb-28">
          ...
          >
            <p className="text-xs uppercase tracking-wider mb-4" style={{ color: '#8AAF8C', fontWeight: 300 }}>
            ...
            {weightLogs.length < 2 ? (
              <div
                ...
                style={{ backgroundColor: '#0D1B10' }}
              >
                <p className="text-xs text-center" style={{ color: '#8AAF8C', fontWeight: 300 }}>
                  {weightLogs.length === 0
              ...
              <ResponsiveContainer width="100%" height={180}>
                <LineChart data={chartData} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#22362A" vertical={false} />
                  <XAxis
                    dataKey="date"
                    tick={{ fontSize: 10, fill: '#8AAF8C' }}
                    tickLine={false}
                    axisLine={false}
                  ...
                  <Line
                    ...
                    strokeWidth={2}
                    dot={false}
                    activeDot={{ r: 4, fill: '#72C45A', strokeWidth: 0 }}
                  />
                </LineChart>
              </ResponsiveContainer>
            )}
              ...
              <div className="grid grid-cols-4 gap-2 mt-4">
                {[
                  ...
                  <div
                    ...
                    style={{ backgroundColor: '#0D1B10' }}
                  >
                    <p className="text-xs mb-1" style={{ color: '#8AAF8C', fontWeight: 300 }}>{label}</p>
                    <p className="text-sm" style={{ color: '#EAF2E6', fontWeight: 600 }}>{value}</p>
                  </div>
                ))}
              </div>
            )}
          ...
          >
            ...
            <div className="flex flex-wrap gap-3 mb-4">
              {Object.entries(STATUS_LABEL).map(([key, label]) => (
                <div key={key} className="flex items-center gap-1.5">
                  ...
                  />
                  <span className="text-xs" style={{ color: '#8AAF8C', fontWeight: 300 }}>{label}</span>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                >
                  <p className="text-xs mb-1" style={{ color: '#8AAF8C', fontWeight: 300 }}>{label}</p>
                  <p className="text-lg" style={{ color: '#72C45A', fontWeight: 600 }}>
                    {value}
                    <span className="text-xs ml-0.5" style={{ fontWeight: 300, color: '#8AAF8C' }}>{unit}</span>
                  </p>
                </div>
              ))}
            </div>
